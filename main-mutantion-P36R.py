...
traj = md.load('MutationConformationsBioEmu/out_mutant_P36R/samples.xtc', top='MutationConformationsBioEmu/out_mutant_P36R/topology.pdb')
...
plt.title('Conformational Profile: RMSF by Residue - MUTANT (P36R) Annexin A11', fontsize=14)
plt.xlabel('Residue Number', fontsize=12)
plt.ylabel('RMSF ($\AA$)', fontsize=12)
plt.grid(True, linestyle='--', alpha=0.6)

plt.savefig('conformational_profile_mutant_p36r.png', dpi=300)
plt.show()
...
plt.title('Conformational Profile: PCA - MUTANT(P36R) Annexin A11', fontsize=14)
plt.xlabel(f'PC1 ({pca.explained_variance_ratio_[0]*100:.1f}%)') 
plt.ylabel(f'PC2 ({pca.explained_variance_ratio_[1]*100:.1f}%)') 
plt.grid(True, alpha=0.3) 
plt.savefig('pca_conformational_space_mutant_p36r.png') 
plt.show()

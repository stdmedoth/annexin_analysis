...
print(f'{wt_coords.shape},{wt_reduced.shape}')

# 3. Processar Mutação e Projetar no Espaço da WT
# Aqui você pode repetir para outras mutações em um loop se quiser
wt2_traj, wt2_coords = get_aligned_coords(
    'luis_felipe/samples.xtc',
    'luis_felipe/topology.pdb',
    ref_traj=wt_traj # Alinhamento cruzado
)
wt2_reduced = pca.transform(wt2_coords) # Projeção
print(f'{wt2_coords.shape},{wt2_reduced.shape}')

# --- VISUALIZAÇÃO ---
plt.figure(figsize=(10, 7))

# Plot WT
plt.scatter(wt_reduced[:, 0], wt_reduced[:, 1], c='blue', alpha=0.4, label='WT Generated by Calisto', s=10)

# Plot Mutação
plt.scatter(wt2_reduced[:, 0], wt2_reduced[:, 1], c='red', alpha=0.4, label=f'WT Generated by Luis Felipe', s=10)

# Estética
plt.title(f'Comparisson of Conformational Space: WT Generated by Calisto vs WT Generated by Luis Felipe', fontsize=14)
plt.xlabel(f'PC1 ({pca.explained_variance_ratio_[0]*100:.1f}%)')
plt.ylabel(f'PC2 ({pca.explained_variance_ratio_[1]*100:.1f}%)')
plt.legend()
plt.grid(True, alpha=0.3)

plt.savefig('pca_comparison_wt_calisto_luis_felipe.png', dpi=300)
plt.show()

...
def compute_idr_to_core_distance_comparison():
    ...
    print("\nFinding distances (SASA and RSA)...")
    distances1 = analyzer.compute_idr_to_exposed_core_dist(variant1_traj)
    ...
    print("\nFinding distances (SASA and RSA)...")
    distances2 = analyzer.compute_idr_to_exposed_core_dist(variant2_traj)
    ...
    visualizer.plot_core_idr_mean_distances_comparisson(
        ...
        title=f"IDR residues distance to CORE (RSA) - {variant1.label} X {variant2.label}",
        filename=f"idr_core_distances_rsa_{variant1.name.lower()}_{variant2.name.lower()}.png",
        show=True
    )
